...
> ⚠️ **교육·연구용입니다. 투자 권유가 아닙니다.** 대시보드에서 거래비용·무위험수익률을 반영한 백테스트를 실행 시점의 데이터로 다시 계산합니다. 공격형·균형형 바스켓과 SPY의 CAGR·MDD·Sharpe 비교는 대시보드 검증 탭의 최신 결과를 기준으로 확인하세요. 초과수익의 상당 부분은 가격 추세추종에서 오며(선행축에 주가 포함) 순수 매크로 알파는 제한적입니다. 단독 매매신호로 쓰지 말고 전술적 자산배분(TAA)의 비중·틸트 다이얼로 사용하세요.
