...
## 전략 백테스트(`strategy`) — 수익+방어 균형 (2006~2026, 비용·리스크 차감)
| 전략 | CAGR | MDD | Sharpe | 연회전 |
|---|---|---|---|---|
| SPY 단순보유(벤치) | **11.1%** | -51% | **0.77** | - |
| **국면틸트(QQQ공격+선택필터)** ★ | **10.2%** | **-36%** | 0.74 | 0.9 |
| SPY 국면타이밍(순수방어) | 6.2% | -21% | 0.71 | 1.5 |
| 섹터 favored 상위4 등가 | 4.5% | -27% | 0.47 | 4.2 |

**결론(데이터 검증):**
1. **순수 방어는 수익을 반토막낸다**(6.2%). 사용자 지침(수익도 중요) 반영 → **국면틸트**: 좋은 국면(회복·성장)엔 고베타 **QQQ로 풀투자**(추세필터는 방어 국면에만), 나쁜 국면에만 빠진다.
2. 결과 → **buy-hold 수익에 근접(10.2 vs 11.1%)하면서 MDD는 -15%p 낮고(-36% vs -51%) Sharpe 동급(0.74)**. 회전율도 0.9로 최저. = 수익과 방어를 동시에.
3. **섹터선택은 여전히 알파 없음** — 국면 베타로 타는 게 최적. → 종목/섹터는 보조, **핵심은 "주식비중·고베타 타이밍"**.
...
- 추천(성장): 자산배분 바스켓 총 주식비중 90%(SPY30/QQQ20/IWM20/EEM10/EFA10 + GLD7/SHY3), 섹터 ETF 모멘텀 상위=XLK·XLE·XLB·XLI, 개별종목=시클리컬 4섹터(산업재/소재/금융/에너지) S&P500 모멘텀 상위 15.

## 지수 = 국면별 자산배분 바스켓 (수익+방어 균형)
지수 추천은 단순 나열이 아니라 **국면별 자산배분 바스켓**(`regime_index_basket`): 미국주식(QQQ/SPY/IWM/DIA) + 지역(EFA=DM/EEM=EM) + 안전자산(TLT/GLD/SHY), 각 목표비중·6M모멘텀·200DMA 상회 표시. 좋은 국면(회복/성장)은 **고베타 QQQ 틸트로 주식 90%+ 풀투자**해 상승을 먹고, 방어 국면에만 안전자산을 늘린다. **총 주식비중**이 RISK 베타(회복/성장 90%→둔화 55%→침체 25%)를 따라가 모델 핵심(위험노출 타이밍)을 노출.
