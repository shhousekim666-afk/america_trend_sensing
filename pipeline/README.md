...
5. **적립식(DCA)에선 결론이 뒤집힌다** — 매달 정액 납입 시: QQQ 적립 9.2x(IRR 18.6%) > SPY 적립 4.8x(13.4%) > 공격형 4.0x(12.1%) > 균형형 2.9x(9.4%). 국면 타이밍이 단순 적립보다 못함. 적립금은 자금이 나중에 쌓이고 폭락장이 곧 싸게 사는 기회인데 방어가 그 기회를 놓치기 때문. → **국면 타이밍은 목돈/기존자산용, 매달 적립금은 QQQ·SPY 꾸준히가 유리**(검증탭 적립식 섹션).
