...
| **공격형 — 권고 바스켓(QQQ중심+침체방어)** ★ | **11.6%** | **-28%** | **0.85** |
| 균형형 — 분산 바스켓(낙폭최소·효율최고) | 9.5% | -25% | 0.78 |
| SPY 국면타이밍(순수방어) | 6.2% | -21% | 0.71 |
| 섹터 favored 상위4 등가 | 4.5% | -27% | 0.47 |

**결론(데이터 검증):**
1. **공격형(권고 바스켓)이 SPY를 CAGR·MDD·Sharpe 3개 모두 초과** — 좋은 국면(회복·성장)+둔화까지 고베타 **QQQ 중심**으로 채워 상승을 먹고(자체 16% 수익엔진), 금·채권 슬리브로 변동성을 잡고, 침체에만 안전자산 최대화. 레버리지 없음.
2. **균형형**은 분산을 더 강화해 낙폭 최소(-25%)·효율 최고(Sharpe 0.78)이나 수익은 SPY 아래. 영상 지역 프레임(성장=이머징/Non-US)을 더 충실히 반영. 위험성향에 따라 선택.
3. **섹터선택은 알파 없음** — 자산(QQQ/SPY) 국면 베타로 타는 게 최적. → 종목/섹터는 보조, **핵심은 "주식비중 + QQQ 틸트 타이밍"**.
