...
5. **적립식(DCA)에선 결론이 뒤집힌다** — 매달 정액 납입 시: QQQ 적립 9.2x(IRR 18.6%/MDD -32%) > **QQQ75/GLD25 7.3x(16.8%/-26%, 추천)** > SPY 4.8x(13.4%/-27%) > 공격형 4.1x(12.3%/-27%) > 균형형 3.0x(9.7%). 국면 타이밍은 적립에 안 맞음(거시 신호 느려 MDD 못 줄임, 폭락=싸게 사는 기회를 방어가 놓침). 효과적인 건 **금(GLD) 상수 슬리브** — QQQ를 안 팔고 위기(2008·2022)에 오르는 금으로 낙폭만 쿠션(QQQ 수익 ~90% 유지, MDD는 QQQ·공격형보다 낮음). 채권 TLT은 2022 동반폭락으로 실패. **한계: 2006~26은 금 유리 레짐, 2013~18 금약세기엔 QQQ 단순적립보다 수익 낮았음.** "QQQ보다 수익↑+낙폭↓ 동시"는 무레버리지 불가(트레이드오프).

## 엔진 핵심(개선 반영)
- 신뢰도: 연속 DI → 4국면 거리 → **softmax 확률**(양자화 제거, 중립 실작동).
- 레벨 게이트: 금리차 역전 중 +기여 차단 / 실업률 4%↓ 악화기여 반감(2024 오신호 방어).
- 침체 트리거 삼중화: 실업률 연속상승 **OR** HY 신용스프레드(BAMLH0A0HYM2) 12M평균 25%↑ **OR** Sahm Rule 실시간(SAHMREALTIME ≥0.5). (선행축 후보 T10Y3M·PERMIT는 PIT/NBER 검증 결과 각각 유해·무익으로 판명돼 채택 안 함.)
- 비대칭 지속성: 침체(방어)는 2개월, 그 외 3개월 확정 → 침체 recall 보강 + 휘프소 억제.
- 추천(성장): 공격형 바스켓 총 주식비중 95%(QQQ45/SPY20/IWM15/EEM8/EFA7 + GLD5), 섹터 ETF 모멘텀 상위=XLK·XLE·XLB·XLI. **개별종목 유니버스 = 섹터 패널과 동일**(양+모멘텀 favored 섹터 = 화면에 뜨는 뜨거운 섹터, 성장이면 IT 포함) → 팩터 랭킹, **음수 스코어 컷 + 섹터당 최대 3종목**.
...
- **PIT(발표시점) 무편향 백테스트 구현·검증 완료**(`cli pit`, FRED_API_KEY 필요): ALFRED vintage 로 각 시점 실제 알려진 값만 써서 국면 재구성. 발표시점·개정 두 경로를 **동일한 실시간 재구성 방식**으로 만들어 값만 비교(2006~, 248개월). **정직한 결과: look-ahead 편향 실재** — ① 침체 탐지 악화(recall 0.45→0.35, 실시간엔 더 늦게 잡음), ② 전략 수익은 vintage에 견고(공격형 9.4↔9.5%, 엣지가 비개정 시장·선행축에 실림), ③ 엄격한 실시간 MDD는 −44~46%로 헤드라인(−28%, 단순 1M-lag)보다 깊음(실시간 방어 느림). *(초기 비교는 인덱싱 아티팩트로 PIT가 나아 보였으나, 동일 방식 재비교로 정정.)*
