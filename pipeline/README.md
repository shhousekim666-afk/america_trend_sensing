...
- **PIT(발표시점) 무편향 백테스트 구현·검증 완료**(`cli pit`, FRED_API_KEY 필요): ALFRED vintage 로 각 시점 실제 알려진 값만 써서 국면 재구성(248개월). 개정 대비 **look-ahead 편향 미미**(F1 0.53→0.61, 공격형 11.4%→12.0%, 방향성 지표라 레벨 개정에 강함). 유일한 비용은 휘프소 증가(0.054→0.093). 시장·비개정 지표는 현재값=발표시점값으로 처리.
